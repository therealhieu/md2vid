...
The live canary root cause is the trusted policy's dependency on the nested workflow event field `github.event.workflow_run.pull_requests[0].head.repo.full_name`. Observer run `30341879923` attempt `5` succeeded. Trusted run `30372951891` passed run fetch, PR correlation, event number/ref/SHA, trusted/observed/live repo schema, live head, and one verified commit; the policy then failed solely with `dependency metadata is malformed`.
