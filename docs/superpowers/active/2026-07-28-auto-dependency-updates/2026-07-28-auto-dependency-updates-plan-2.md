...
## Task 5.4 canary evidence — 2026-07-28

The live canary root cause is the trusted policy's dependency on the nested workflow event field `github.event.workflow_run.pull_requests[0].head.repo.full_name`. Observer run `30341879923` attempt `3` succeeded. Trusted run `30361571480` successfully fetched trusted run, PR, and commit state and reached policy validation, proving the prior Actions API and PR-correlation fixes. Its environment had `EVENT_PR_NUMBER=25`, `EVENT_HEAD_REF` populated, and `EVENT_HEAD_SHA` populated, but `EVENT_HEAD_REPOSITORY` was empty from `${{ github.event.workflow_run.pull_requests[0].head.repo.full_name }}`. The policy failed solely with `workflow_run PR identity is invalid` because it required that empty event field to equal `therealhieu/md2vid`.

The implementation must keep the event PR number/ref/SHA requirements, but it must not require nested event head repository. Repository identity remains mandatory and must be independently validated through fetched trusted state: `run.repository.full_name`, `run.pull_requests[0].head.repo.full_name`, `run.pull_requests[0].base.repo.full_name`, and live PR `head.repo.full_name`/`base.repo.full_name`. Tests must first go RED against the current workflow by proving an absent event head repo is accepted only when trusted run and live PR repository identity are exact, and by proving wrong observed or live repository identity still fails.
