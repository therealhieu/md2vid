...
>
> **Treatments 1–6** are the PR-shaped set (diffs, stats, quotes). **Treatments 7–11** (Structure,
> Flow, Contrast, Enumerate, Matrix) are the expository set — how source *knowledge* (diagrams,
> processes, comparisons, lists, tables) becomes a frame. For the decision logic that maps a piece
> of source content to a treatment, see **`knowledge-expression.md`**.
...
> **Treatments 7–11 are the expository set.** Treatments 1–6 were authored for PR / product videos (a diff, a stat, a quote). Explainers derived from documents, articles, and technical topics lean on the five below — they express *concepts, structures, comparisons, sequences, lists, and tables*, which the first six do not cover. The knowledge-type → treatment routing lives in the companion **`knowledge-expression.md`**; this section defines each treatment's atoms + reveal. The universal law across all eleven still holds: **one focal, one coral moment, open with a kicker-spike.**

### 7 · Structure (diagram / schematic · move: redrawn hairline nodes + one coral trace · cream)

**Ground** `{colors.cream}` (navy only for a data payload — leaf pointers, inverted-index rows). **Composes** kicker-spike, `card-hairline` node cards / tokens / a hub, `section-rule` edges (SVG stroke-draw), mono node labels, one coral trace overlay. **Focal** the source diagram **redrawn from theme atoms** — never a pasted image or Mermaid render. **Chrome** mono labels on nodes. **Accent** the single coral path — a descent trace, a bypass arc, a slash, a highlighted subset. **Reveal** build-then-trace: assemble the whole structure in ink/tile first (top-down or L→R), *then* one coral element draws the "read this" traversal node-by-node on the VO cue. **Fixed** redrawn hairline nodes on cream, structure-before-coral, one coral path. **Free** the diagram, layout, how the trace moves. **Density** free-to-dense. **Exemplars** B-tree descent, covering-index bypass, inverted-index map, constellation hub, value→rows mapping.

### 8 · Flow (process / sequence · move: virtual camera pans an oversized world · cream)

**Ground** `{colors.cream}` world **larger than 1920×1080** (wider or taller), viewed through a single-wrapper virtual camera (deterministic `translate`/`scale`). **Composes** kicker-spike, tile station cards, mono step-labels, ink connector strokes + arrowheads, a coral decision diamond. **Focal** the pipeline — the camera pans/steps station→station, each connector drawing on cue. **Chrome** mono step numbers. **Accent** coral decision diamonds (the pivots) + a coral pulse on the chosen/critical branch. **Reveal** camera moves to each station on its spoken cue → connector draws (`getTotalLength` dash) → node pops (`power3.out`) → fork branches draw last; hold on the fork. **Pan direction encodes meaning** — down for a write stack, across for a pipeline. **Fixed** one camera over one world (no cuts), coral diamonds, drawn connectors. **Free** world shape, pan path, station count. **Density** dense. **Exemplars** query planner, index-design workflow, vertical write-cost stack, two-stage spatial filter.

### 9 · Contrast (comparison / trade-off · move: side-by-side or emphasis-swap · cream)

**Ground** `{colors.cream}` (a full `{colors.navy}` ground is allowed here for a rhythm/correction beat — the one sanctioned non-code navy). **Composes** kicker-spike, two mirrored hairline panels (or two stacked paths, or one line that swaps emphasis), per-row coral emphasis, a coral `coral-callout` stamp/rule/band for the resolution. **Focal** the two sides held together — panels book-open from opposite wings, rows fill in **matched pairs** on cue. **Chrome** mono panel headers. **Accent** coral marks **one side at a time** (traveling), then the coral resolution lands (stamp / rule strip / bypass arc — the one earned `back.out` overshoot). **Reveal** empty panels enter → paired rows fill → coral travels side-to-side → coral resolution pops. Green/red verdict badges (`#5db872` / warn-red) are permitted as **fixed decoration** — they never compete with the single traveling coral. **Fixed** spatial split, pairwise reveal, coral on the resolution. **Free** panel geometry, what resolves. **Density** free-to-dense. **Exemplars** without/with index + MUST-MAINTAIN stamp, index-everything → real-queries swap, leftmost-prefix ✓/✓/✗ + rule strip.

### 10 · Enumerate (list / checklist · move: VO-paced accumulation · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, `card-hairline` tiles or rail rows, mono index numbers, a coral ring / "!" badge / drawn tick / navy SQL fragment per item. **Focal** cards or rows **accumulating one-per-cue** into a grid (2×2, 3×2) or a vertical rail — never all-at-once. **Chrome** mono index numbers, an optional "part N of 2" pill. **Accent** exactly **one coral item live** — the ring/badge/tick/fragment rides only the currently-named item, then hands off to the next; settles neutral (or the last item keeps coral at rest). **Reveal** (optional) empty slots cascade first, then items snap in on cue (`back.out(1.3)` = snap, `power3.out` = calm); coral hands down the list. **Vary layout between consecutive list frames** so two lists never read as the same slide. **Fixed** VO-paced accumulation, one coral at a time, layout variety. **Free** grid vs rail, item count, snap vs calm. **Density** free-to-dense. **Exemplars** query shapes 2×2, anti-patterns 3×2, need→code recipes, checklist single-rail vs two-column.

### 11 · Matrix (table / decision matrix · move: spotlight-revealed rows · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, redrawn grid rows with a left coral `.bar`, mono column heads, green/red badges (fixed decoration), a coral link arrow (SVG stroke-draw) between columns. **Focal** a **redrawn** table — rows/pairs pre-placed but **dim**, never dumped cold at t=0. **Chrome** mono column headers. **Accent** one coral **spotlight travels** row-by-row on the spoken cue (a coral bar / row wash / link arrow); a verdict or bridge lands last. **Reveal** header + rule → rows arrive staggered → coral spotlight travels one row at a time → verdict/bridge. **Fixed** redrawn rows (never a pasted table), staggered arrival, single traveling coral spotlight. **Free** column count, badge scheme, the verdict. **Density** dense. **Exemplars** predicate-fit Good/Poor, cardinality table, need→type decision matrix.

## Rhythm (cross-frame — the film, not the frame)

The six PR treatments assume a short film; an expository video runs 20+ frames and needs rhythm rules that live *between* frames:

- **The 3-scene spine is universal.** Every frame: (1) headline + focal anchor by t ≤ 0.5s, (2) beat-by-beat reveal — **one spoken sentence → one visual reveal** (each node / row / card / code line timed to its VO cue), (3) a held landing. No cold dump at t=0; no reveal that outruns the narration.
- **Allocate breather frames on purpose.** Not every frame is dense. Against busy neighbors, place a calm frame that is *revealed then held still* (a two-card concept, a callback line). Density is free per frame, but the sequence needs troughs or it reads as relentless.
- **Vary framing between consecutive same-type frames.** Two adjacent lists, tables, or flows must not share a layout — single-rail vs two-column, horizontal vs vertical pan, 2×2 vs 3×2. Same treatment, different geometry.
- **Every frame ends on a ≥0.5s held landing.** The final beat carries no reveal — a breath before the next frame begins, so the sequence never reads as a continuous scroll.
- **Frame 1 opens with an agenda; the final frame recaps it.** The intro states what the video covers and lists the beats ahead (Cover → Enumerate); the recap mirrors those items before the callback close (Enumerate → Closing).
- **Bookend with a callback.** Reuse the cover's hero word / ground motif in the closing frame to rhyme the open and close.
- **Coral hand-off is the enforcement mechanism.** Across a multi-item reveal, coral is *handed* item-to-item (animate the previous back to neutral as the next lights). The "coral by default in CSS, revert on next cue" inversion guarantees exactly-one-coral under arbitrary seek.
...
- Ration **coral to one moment per frame** — CTA, inline link, OR full-bleed band. Across a multi-item reveal, **hand coral off** item-to-item so only one is ever live.
- Elevate with a **1px hairline** + at most one soft warm shadow; reserve **warm navy** for the code/terminal surface (or one sanctioned Contrast rhythm beat).
...
- **Redraw** every source diagram, table, and chart from theme atoms; assemble structure in ink first, then one coral element traces the "read this" motion.
- Time every reveal to a **VO cue**; vary framing between consecutive same-type frames; allocate deliberate breather frames.

### Don't

- No pure white, no cool gray, no pure black; no fourth brand hue (navy is structural, syntax colors + green/red verdicts are decoration).
...
- **Never paste a raw diagram, screenshot, Mermaid render, or table** — redraw it. No cold dump of a full table/list at t=0. No reveal that outruns the VO. No two adjacent frames sharing a layout.
